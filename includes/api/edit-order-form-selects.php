...
function cpd_update_print_order_ajax() {
    ...
    $selected_name = $fields['selected_print_house'] ?? $results[0]['print_house'];
    $selected = array_values(array_filter($results, fn($r) => $r['print_house'] === $selected_name))[0] ?? $results[0];
...
add_shortcode('edit_print_order_form', function () {
    if (!is_singular('print_order')) return '';
    global $post;
    $post_id = $post->ID;

    $sizes = ['A4', 'A5', '148x210', '210x280'];
    $bindings = ['Perfect Bound', 'Thread Sewn', 'Stapled'];
    $interiors = ['1/1 Black', '4/4 Color', '4/0 Euro'];
    ...
    <div style="max-width:600px;margin:3em auto 4em;padding:2em;border:1px solid #ddd;border-radius:16px;background:#fdfdfd;box-shadow:0 0 10px rgba(0,0,0,0.05);">
        ...
        <form id="edit-print-order-form" method="post">
            ...
            <div style="margin-bottom:1em;">
                ...
                <select name="fields[book_size]" style="width:100%;padding:0.5em;border-radius:6px;border:1px solid #ccc;">
                    <?php
                    $current = get_field('book_size', $post_id);
            ...
            <div style="margin-bottom:1em;">
                ...
                <select name="fields[binding_method]" style="width:100%;padding:0.5em;border-radius:6px;border:1px solid #ccc;">
                    <?php
                    $current = get_field('binding_method', $post_id);
            ...
            <div style="margin-bottom:1.5em;">
                ...
                <select name="fields[interior_print]" style="width:100%;padding:0.5em;border-radius:6px;border:1px solid #ccc;">
                    <?php
                    $current = get_field('interior_print', $post_id);
                    foreach ($interiors as $opt) {
                        $selected = ($opt === $current) ? 'selected' : '';
                        echo '<option value="' . esc_attr($opt) . '" ' . $selected . '>' . esc_html($opt) . '</option>';
                    }
                    ?>
            ...
            <div style="margin-bottom:2em;">
                <label for="selected_print_house">Select another Print House:</label>
                <select name="fields[selected_print_house]" id="selected_print_house" style="width:100%;padding:0.5em;border-radius:6px;border:1px solid #ccc;">
                    <?php
                    $all = function_exists('ppp_get_all_print_house_names') ? ppp_get_all_print_house_names() : [];
                    $current = get_field('selected_print_house', $post_id);
                    foreach ($all as $name) {
